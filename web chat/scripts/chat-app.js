...
class ChatApp {
   ...
   */
  initListeners() {
    this._componenets.appBranch.on(
      APP_EVENTS.PROFILE_BTN_CLICK,
      this._onProfileBtnClick.bind(this)
    )
    this._componenets.chatsList.on(
      APP_EVENTS.CHAT_SELECTED,
      this._onChatSelected.bind(this)
    )
    this._componenets.chatBox.on(
      APP_EVENTS.AUTHED_USER_NEW_MESSAGE,
      this._onAuthedUserNewMessages.bind(this)
    )
    this._componenets.chatBox.on(
      APP_EVENTS.CHAT_BOX_BACK_CLICKED,
      this._onChatBoxBack.bind(this)
    )
  }
  ...
  newMessage(msg) {
    ...
    this.assert(
      msg && msg.time && msg.sender && msg.text && msg.toChat,
  ...
  _onChatSelected({ detail }) {
    ...
    const chatMessaged = this._messages.filter(
      (m) => m.sender === detail.id || m.toChat === detail.id
    )
    ...
    if (
      this._componenets.chatBox.activeChat &&
      this._componenets.chatBox.activeChat.id === this.activeChat.id
    ) {
