class ChatsList extends Component {
  ...
  initListeners() {
    ...
    this.on(
      APP_EVENTS.NEW_MESSAGE_RECEIVE,
      this._onNewMessageReceive.bind(this)
    )
   ...
   */
  _onSearch(e) {
    if (this._searchDebounceFlag) clearTimeout(this._searchDebounceFlag)
  ...
  generateChatListItem(chat) {
    ...
    if (chat.online) chatListItem.setAttribute("online", "")
  ...
  appendChatToList(chat, appendFirst = false) {
    ...
    if (
      ~chat.name.toLowerCase().indexOf(searchTrend) ||
      ...
      if (!appendFirst) {
        ...
        // append as the first child of the list
