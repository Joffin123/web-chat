class ChatListItem extends Component {
   ...
   */
  set unread(value) {
    if (value) this.setAttribute("unread", "")
    else this.removeAttribute("unread")
  }
   ...
   */
  set online(value) {
    if (value) this.setAttribute("online", "")
  ...
  render() {
    ...
    if (!this.getAttribute("avatar")) {
      ...
      this.shadowRoot.querySelector(".char-avatar").innerText = name.substr(
        0,
        1
      )
    }
    ...
    for (let attr of this.attributes) {
      ...
      if (target) target.innerText = attr.value
