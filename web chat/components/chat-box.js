class ChatBox extends Component {
  ...
  attributeChangedCallback(attrName, oldValue, newValue) {
    ...
    if (attrName === "readonly") this.checkNewMessageBoxVisibility()
  }
  ...
  initListeners() {
    ...
    this._newMessageBox.on(
      APP_EVENTS.AUTHED_USER_NEW_MESSAGE,
      this._onAuthedMessageReceive.bind(this)
    )

    // control the visibility and the behavior of scrollToBottom button in chats list
    this._chatList.addEventListener(
      "scroll",
      this.checkScrollToBottomBtnVisibility.bind(this)
    )
    this._scrollToBottomBtn.addEventListener(
      "click",
      this.scrollToEnd.bind(this)
    )

    this._activeChatElm.on(
      APP_EVENTS.CHAT_BOX_BACK_CLICKED,
      this._onBackBtnClicked.bind(this)
    )
  }
   ...
   */
  removeListeners() {
    this._newMessageBox.off(
      APP_EVENTS.AUTHED_USER_NEW_MESSAGE,
      this._onAuthedMessageReceive.bind(this)
    )
    this.off(APP_EVENTS.USER_SIGN_IN, this._userSignIn.bind(this))
    this._scrollToBottomBtn.removeEventListener(
      "click",
      this.scrollToEnd.bind(this)
    )
    this._chatList.removeEventListener(
      "scroll",
      this.checkScrollToBottomBtnVisibility.bind(this)
    )
  }
  ...
  renderMessage({ sender, text, audio, time }, forceScrollToEnd = false) {
    ...
    if (!sender || (!text && !audio) || !time || !(time instanceof Date)) return
    ...
    if (this.lastMessage && isSameSender) this.lastMessage.isLastInGroup = false
    ...
    const isLastMessageInView =
      this._chatList.scrollTop >=
      this._chatList.scrollHeight - this._chatList.clientHeight
    ...
    if (isFromAuthedUser) this._newMessageBox.clear()

    if (isLastMessageInView || forceScrollToEnd) this.scrollToEnd()
  ...
  _isMessageForDifferentDay(time) {
    if (!this.lastMessage || !time) return false
  ...
  _appendDateToChatList(time) {
    ...
    if (time.toDateString() === new Date().toDateString()) dayTitle = "Today"
  ...
  renderChatBoxHeader() {
    ...
    if (this._activeChat.online) activeChatNode.setAttribute("online", "")
    else activeChatNode.removeAttribute("online")
  }
